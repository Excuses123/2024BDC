...
def cmd_args():
    ...
    parser.add_argument("--num_sta", type=int, default=240, help="验证的station个数")
...
if __name__ == '__main__':
    ...
    # 随机240个station进行验证
    sta = np.array([random.randint(0, num_station) for _ in range(args.num_sta)])
    ...
    a_val = np.concatenate([a_val[None, i//3: i//3+56+24, :, :, sta] for i in win], axis=0)  # [71, 56+24, 4, 9, S]
    b_val = np.concatenate([b_val[None, i: i+168+72, sta, :] for i in win], axis=0)  # [71, 168+72, S, 1]
    c_val = np.concatenate([c_val[None, i: i+168+72, sta, :] for i in win], axis=0)  # [71, 168+72, S, 1]
